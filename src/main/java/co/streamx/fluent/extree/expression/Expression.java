...
//                    first.getResultType().toString() + " != " + second.getResultType().toString());
    ...
    /**
     * Creates a {@link DelegateExpression} as a method receiving the specified
     * {@code arguments}, returning the {@code resultType} and having delegate to
     * the implementation.
     * 
     * @param resultType The method return value.
     * @param delegate   The method implementation. The delegate resultType must be
     *                   {@link InvocableExpression}.
     * @param parameters The method parameters.
     * @return A {@link DelegateExpression} as a method receiving the specified
     *         {@code arguments}, returning the {@code resultType} and having
     *         delegate to the implementation.
     */
    public static DelegateExpression delegate(Class<?> resultType,
                                              Expression delegate,
                                              List<ParameterExpression> parameters) {
    ...
    /**
     * Creates a {@link BlockExpression}
     * 
     * @param resultType  The result type of the block.
     * @param expressions The expressions in the block.
     * @return The created {@link BlockExpression}
     */
    public static BlockExpression block(Class<?> resultType,
                                        List<Expression> expressions) {
        return new BlockExpression(resultType, expressions);
    }

    /**
     * Creates a {@link NewArrayInitExpression} that represents creating a
     * one-dimensional array and initializing it from a list of elements.
     * 
     * @param type         {@link Class} that represents the component type of the
     *                     array.
     * @param initializers {@link Expression} objects used to populate the array.
     * @return The created {@link NewArrayInitExpression}
     */
    public static NewArrayInitExpression newArrayInit(Class<?> type,
                                                      List<Expression> initializers) {
        return new NewArrayInitExpression(type, initializers);
    }

    /**
     * Creates a {@link MemberExpression} that represents accessing a static field
     * given the name of the field.
     * 
     * @param type The {@link Class} that specifies the type that contains the
     ...
     */
    public static MemberExpression get(Class<?> type,
                                       String name)
    ...
    /**
     * Creates a {@link MemberExpression} that represents accessing an instance
     * field given the name of the field.
     * 
     * @param instance An {@link Expression} whose {@code getResultType()} value
     *                 will be searched for a specific field.
     * @param name     The name of a field.
     * @return A {@link InvocationExpression} that represents accessing an instance
     *         field given the name of the field.
     * @throws NoSuchFieldException if a field with the specified name is not found.
     */
    public static MemberExpression get(Expression instance,
                                       String name)
            throws NoSuchFieldException {
     ...
     * @return An {@link InvocationExpression} that represents accessing an instance
     *         field.
     */
    public static MemberExpression get(Expression instance,
                                       Field field) {
        return member(ExpressionType.FieldAccess, instance, field, field.getType(),
                Collections.<ParameterExpression>emptyList());
    }

    /**
     * Creates an {@link InvocationExpression} that represents a call to an instance
     * method, or {@link UnaryExpression} in case of boxing.
     * 
     * @param instance  An {@link Expression} whose {@code getResultType()} value
     *                  will be searched for a specific method.
     * @param method    The {@link Method} to be called.
     * @param arguments An array of {@link Expression} objects that represent the
     *                  arguments to the method.
     * @return An {@link InvocationExpression} that has the {@link ExpressionType}
     *         method equal to Invoke or Convert in case of boxing.
     */
    public static Expression invoke(Expression instance,
                                    Method method,
    ...
    public static Expression invoke(Expression instance,
        ...
        if (instance != null) {
            ...
            Class<?> boxer;
            Expression e;
            if (arguments.size() == 1 && (e = arguments.get(0)).getResultType().isPrimitive()
        ...
        if (method.isSynthetic()) {
            ...
            LambdaExpression<?> lambdaExpression = ExpressionClassCracker.get()
                    .lambdaFromFileSystem(actualInstance, method, null);
            return invoke(lambdaExpression, arguments);// arguments.get(0).accept(Interpreter.Instance).apply(null).getClass().isSynthetic()
        }

        return invoke(
                member(ExpressionType.MethodAccess, instance, method, method.getReturnType(), getParameters(method)),
                arguments);
    }
     ...
     * method.
     * 
     * @param method    An {@link InvocableExpression} which encapsulates method to
     *                  be called.
     * @param arguments An array of {@link Expression} objects that represent the
     *                  arguments to the method.
     * @return An {@link InvocationExpression} that has the {@link ExpressionType}
    ...
    /**
     * Creates a {@link InvocationExpression} that represents calling the specified
     * constructor.
     * 
     * @param method    The constructor to invoke.
     * @param arguments The constructor arguments.
     * @return A {@link InvocationExpression} that represents calling the specified
     *         constructor.
     */
    ...
    public static InvocationExpression newInstance(Constructor<?> method,
                                                   List<Expression> arguments) {
        return invoke(member(ExpressionType.New, null, method, method.getDeclaringClass(), getParameters(method)),
                arguments);
    }
    ...
    /**
     * Creates an {@link InvocationExpression} that represents a call to an instance
     * method by calling the appropriate factory method, or {@link UnaryExpression}
     * in case of boxing.
     * 
     * @param instance       An {@link Expression} whose {@code getResultType()}
     *                       value will be searched for a specific method.
     * @param name           The name of the method.
     * @param parameterTypes An array of {@link Class} objects that specify the type
     *                       of parameters of the method.
     * @param arguments      An array of {@link Expression} objects that represent
    ...
    /**
     * Creates an {@link InvocationExpression} that represents a call to a static
     * method by calling the appropriate factory method, or {@link UnaryExpression}
     * in case of boxing.
     * 
     * @param type           The {@link Class} that specifies the type that contains
     ...
     * @throws NoSuchMethodException if a matching method is not found.
     */
    public static Expression invoke(Class<?> type,
                                    String name,
                                    Class<?>[] parameterTypes,
                                    Expression... arguments)
            throws NoSuchMethodException {
    ...
    private static Method getDeclaredMethod(Class<?> clazz,
        ...
        for (;;) {
            try {
                return tmpClass.getMethod(name, parameterTypes);
    ...
    /**
     * Creates a {@link BinaryExpression} that represents a conditional operation,
